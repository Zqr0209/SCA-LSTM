...
class HydroReader(AbstractReader):
    camels_root = None  # needs to be set in class method "init_root"
    forcing_root = None
    discharge_root = None
    # features = ["prcp", "tmax", "tmin", "aet", "pet"]  # 巴西气象数据
    # features = ["prcp", "srad", "tmax", "tmin", "vprp"] # 美国气象数据
    features = ["prcp", "srad", "tmax", "tmin", "vprp", "aet", "pet"] # 澳大利亚气象数据
    # features = ["rainfall"]  # 唐乃亥气象数据
    target = ["runoff"]

    @classmethod
    def init_root(cls, camels_root):  # often be rewritten
        cls.camels_root = Path(camels_root)
        cls.features_root = cls.camels_root / "forcing"  # 气象数据
        cls.target_root = cls.camels_root / "runoff"  # 径流数据
    ...
    # Loading meteorological data 加载气象数据
    def _load_forcing(self):
        files = list(self.features_root.glob(f"**/{self.basin}.csv"))
        if len(files) == 0:
        ...
        df.index = pd.to_datetime(df['date'], format="%Y/%m/%d")  # 时间戳做索引
        return df[self.features]

    # Loading runoff data 加载径流数据
    def _load_discharge(self):
        files = list(self.target_root.glob(f"**/{self.basin}*.csv"))
        if len(files) == 0:
...
class HydroReaderFactory:
    ...
    @staticmethod
    def get_hydro_reader(camels_root, basin):
        HydroReader.init_root(camels_root)
        reader = HydroReader(basin)
...
class CamelsDataset(Dataset):
    ...
    def _load_data(self):
        ...
        for idx, basin in enumerate(self.basins_list):
            ...
            reader = HydroReaderFactory.get_hydro_reader(self.camels_root, basin)
...
class StaticReader(AbstractStaticReader):
    ...
    def __init__(self, camels_root):
        ...
        # self.static_file_path = Path("/data1/zqr/RR-Former/static_attributes.csv") # 巴西的静态属性
        # self.static_file_path = Path("/data2/zqr/Uniform-CAMELS/CAMELS-AUS/static/static_attributes.csv")  # 澳大利亚的静态属性
        self.static_file_path = Path("/data2/zqr/Uniform-CAMELS/CAMELS-US/static/static_attributes.csv")  # 美国的静态属性
...
class CamelsDatasetWithStatic(CamelsDataset):
    ...
    def __init__(self, camels_root: str, basins_list: list, past_len: int, pred_len: int, stage: str,
                 dates: list, x_mean=None, y_mean=None, x_std=None, y_std=None, y_stds_dict=None, use_runoff=True):
...
class CamelsDatasetWithoutStatic(CamelsDataset):
    """CAMELS dataset with static attributes injected into serialized sequences.
    ...
    def _load_data(self):
        ...
        for idx, basin in enumerate(self.basins_list):
            ...
            if not self.use_runoff:
                ...
                '''''''''
                df_y = df_y[self.dates[0] - pd.DateOffset(months=1): self.dates[1]]  # NOTE:预备工作，最大范围

                df_runoff = df_y[self.dates[0] - pd.DateOffset(months=1):df_y.index[-1] - pd.DateOffset(months=1)]

                df_x = df_x[df_runoff.index[0] + pd.DateOffset(months=1):self.dates[1]]
                df_y = df_y[df_runoff.index[0] + pd.DateOffset(months=1):self.dates[1]]
                '''''''''
