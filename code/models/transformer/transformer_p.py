...
class TransformerP(nn.Module):
    def __init__(self):
        ...
        self.src_linear = nn.Linear(src_size // 10, d_model)
        self.tgt_linear = nn.Linear(tgt_size // 10, d_model)
    ...
    def forward(self, src, tgt):
        src = src.unfold(dimension=-1, size=10, step=10)
        src = src.permute(0, 1, 3, 2)
        tgt = tgt.unfold(dimension=-1, size=10, step=10)
