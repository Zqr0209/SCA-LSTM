...
def cacl_ae(obs, sim):
    ...
    ae = np.mean(np.abs(obs - sim) / obs, axis=0)
    ae_mean = np.mean(ae)
    return ae_mean, ae[:, 0]


def calc_logNSE(obs: np.array, sim: np.array) -> np.array:

    # 将真实值和预测值之中的非正数置为10^-6，防止对数运算中出现无穷
    for i in obs:
        if i[0] <= 0:
            i[0] = 1E-6
    ...
    denominator = np.sum((np.log(obs) - np.log(np.mean(obs, axis=0))) ** 2, axis=0)
    numerator = np.sum((np.log(sim) - np.log(obs)) ** 2, axis=0)
    logNSE = 1 - numerator / denominator

    # lognse为负数的话没有现实意义，所以置0
    if logNSE < 0:
        logNSE[0][0] = 0

    logNSE_mean = np.mean(logNSE)
    # Return mean NSE, and NSE of all locations, respectively
    return logNSE_mean, logNSE[:, 0]
